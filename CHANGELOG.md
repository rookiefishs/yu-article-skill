...
- Security: 从本地和 GitHub `main` 的可达历史中彻底移除旧第三方 TTS 脚本及其会话凭据。
- Security: 移除包含硬编码会话凭据的旧第三方 TTS 脚本，统一使用从 `MIMO_API_KEY` 读取凭据的 MiMo TTS。
- Fix: 视频模板改为动态发现 MiMo 脚本、Puppeteer 和 Chrome，不再依赖本机绝对路径或 npm 缓存路径。
- Fix: 2 号视频模板按文章目录动态生成成片文件名，不再写死具体一期标题。
- Docs: 统一“未指定输出形式默认长文”和“视频未指定视觉方向使用默认规范”的行为。

## 2026/08/04

- Docs: 补充图文照片渲染脚本硬规则，要求内容页逐张隔离到 `1080 x 1440` 视口后整页截图，避免长页面离屏 `.card` 截图导致第 3 张出现横向深色遮挡。

## 2026/07/22

- Docs: 补充图文照片生成规范，明确内容页序号从 `01` 连续递增、封面不显示序号，并要求主配图区域水平居中且通过边界检查验证。
- Refactor: 将 Skill 元信息描述改为 YAML 折叠块，避免长描述单行过长。

## 2026/06/08

- Assets: 更新内置默认封面参考图，来源为 `短视频运营/内容策划/101-Skills推荐第4期：通过写代码的方式生成视频/image/封面图.png`。

## 2026/06/03

- Docs: 将图文照片默认主题从暗黑调整为明亮，并明确暗黑风格仅在用户指定时启用。

## 2026/06/02

- Feat: 新增 2 号 4:3 视频规范与对应视频工程模板，用于指定横版浅色科技图文风视频时复用。
- Docs: 在视频输出与生产流程中补充 2 号模板选择规则，并将默认视频规范与 2 号规范明确区分。
- Refactor: 收紧 MiMo 冰糖配音文本规则，默认使用无标点、无格式符号的纯文本旁白，并让脚本清理旧格式标签与标点。
