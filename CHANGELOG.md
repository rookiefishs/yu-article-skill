...
- Docs: 将视频配音默认路线调整为小米 MiMo `mimo-v2.5-tts` 的 `冰糖` 音色，微软 TTS 改为明确指定或降级备用路线。

## 2026/05/21

- Docs: 补充图文配图与封面提示词尺寸规范，正文配图固定为 `2048 x 1093`，封面图固定为 `1536 x 2048`，并要求提示词中显式写出图片尺寸。
- Feat: 新增小米 MiMo 冰糖音色 TTS 脚本，并在配音规范中补充 MiMo TTS 使用方式、默认模型、音色、接口和环境变量说明。

## 2026/05/19

- Docs: 为视频流程补充时长限制，默认按 1 分钟左右规划；除非用户明确要求长视频，最好不要超过 2 分钟，并同步约束拆页、配音字数与交付验收。
